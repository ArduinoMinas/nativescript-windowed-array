...
import * as observableLocalDef from ".";
...
export class ObservableWindowedArray<T>
  extends observable.Observable
  implements observableLocalDef.ObservableWindowedArray<T> {

  private _window: number = 1;
  ...
  public constructor() {
    ...
    if (arguments.length === 1 && Array.isArray(arguments[0])) {
      this._array = new Array<T>(...(arguments[0] as Array<T>));
    }
    else {
  ...
  public set offset(offset: number) {
    let old: number = this._offset;
    this._offset = offset;
    this._notifyOffsetChange(old);
  }

  public getItem(index: number): T {
    let localIdx = this.offset + index;
    if (localIdx >= this._array.length) localIdx = this._array.length - 1;
    return this._array[localIdx];
  }

  public setItem(index: number, value: T) {
    let localIdx = this.offset + index;
    if (localIdx >= this._array.length) localIdx = this._array.length - 1;
    let oldValue = this._array[localIdx];
    this._array[localIdx] = value;
  ...
  public get length(): number {
    if (this._window > 0 && this._window < this._array.length)
      return this._window;
    else
  ...
  public set length(value: number) {
    if (types.isNumber(value) && this._array && this.length !== value) {
      this.splice(value, this._array.length - value);
      if (!this._window || (this._window > this._array.length && this._array.length)) this._window = this._array.length
    }
  }
  ...
  public push(): number {
    ...
    if (arguments.length === 1 && Array.isArray(arguments[0])) {
      const source: Array<T> = <Array<T>>arguments[0];
      this._array.push(...source);
      if (this._array.length > this.window)
        this.offset += source.length;
      this._addArgs.addedCount = source.length
    }
    else {
      this._array.push.apply(this._array, arguments);
      if (this._array.length > this.window)
        this.offset += arguments.length;
      this._addArgs.addedCount = arguments.length
    }
  ...
  protected _notifyLengthChange() {
    const lengthChangedData = {
      eventName: observable.Observable.propertyChangeEvent,
      object: this,
      propertyName: "length",
      value: this._array.length,
      oldValue: undefined
    };

  }

  protected _notifyOffsetChange(oldValue: number) {
    const lengthChangedData = {
      eventName: observable.Observable.propertyChangeEvent,
      object: this,
      propertyName: "offset",
  ...
  splice(start: number, deleteCount?: number): T[] {
    ...
    this.notify(<observableLocalDef.ChangedData<T>>{
...
export interface ObservableWindowedArray<T> extends observableLocalDef.ObservableArrayDef<T> {
